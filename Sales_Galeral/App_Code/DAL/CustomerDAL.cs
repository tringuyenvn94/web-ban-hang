    ...
    public bool Update_Customer(int id, string usename, string pass, string name, bool gender, string phone, string address, string email)
    {
        try
        {
            if (id != 0)
            {
                ...
                pr_Pass.Value = pass;
                SqlParameter pr_Name = command.Parameters.Add("@Name", SqlDbType.VarChar);
                pr_Name.Value = name;
                SqlParameter pr_Gender = command.Parameters.Add("@Gender", SqlDbType.Bit);
                pr_Gender.Value = gender;
                SqlParameter pr_Phone = command.Parameters.Add("@Phone", SqlDbType.VarChar);
                pr_Phone.Value = phone;
                SqlParameter pr_Address = command.Parameters.Add("@Address", SqlDbType.VarChar);
                pr_Address.Value = address;
                SqlParameter pr_Email = command.Parameters.Add("@Email", SqlDbType.VarChar);
